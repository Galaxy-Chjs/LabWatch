...
| 10.15 | The publisher id in the manifest matches the portal | ❌ two failed attempts, both instructive. (a) The CLI refused the first upload as "suspicious content" although the manifest was correct. (b) I then misread the Manage page header `chjs (galaxy-chjs)` as `id (display-name)`, changed the manifest to `chjs`, and the portal answered with the decisive message: *Publisher ID 'chjs' … should match the publisher ID 'galaxy-chjs'*. Reverted to `galaxy-chjs`. The lesson is in RELEASING.md: when the portal names the expected ID, believe the error, not the page layout. |
...
| 10.15 | 清单中的发布者 ID 与门户一致 | ❌ 两次失败，都有价值。(a) 第一次命令行上传虽然清单正确，却被判为"可疑内容"。(b) 我随后把 Manage 页的 `chjs (galaxy-chjs)` 误读成 `ID (显示名)`，把清单改成 `chjs`，门户随即给出决定性报错：*Publisher ID 'chjs' … should match the publisher ID 'galaxy-chjs'*。已改回 `galaxy-chjs`。教训记在 RELEASING.md：门户报错里点名了期望的 ID 时，以报错为准，不要靠页面排版猜。 |
